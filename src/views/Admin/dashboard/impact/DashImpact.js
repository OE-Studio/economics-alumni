...
const ImpactComp = ({
  ...
  const updateStatus = (status) => {
    const options = {
      ...
      url: `${baseURL}/impact/update-status`,
      method: "POST",
      data: {
        uuid: id,
        status: status,
    ...
    <div
      ...
      <div className="flex flex-col space-y-4 md:space-y-0 md:flex-row md:justify-between w-full">
        <div className="space-y-2">
          {impactType === "Individual" && <p className="text-base font-medium">Individual</p>}
          {impactType === "Set of" && <p className="text-base font-medium">Set of {impactSetYear}</p>}
        ...
        <div className="flex md:flex-col md:items-end justify-between">
          ...
          <div className=" py-1.5 px-1 cursor-pointer relative">
            ...
            {toggle && (
              <div
                className={`editPopup absolute bg-white shadow-xl flex flex-col right-0 p-2 space-y-1.5 z-10 ${last && "bottom-10"
                  }`}
                ref={popUp}
              >
                <div
...
const DashImpact = () => {
  ...
  const loadedData = useSelector((state) => state.impact);
  let loadedDataList;
    ...
    const renderList = (List) => {
      loadedDataList = List.map((trainings, index, list) => {
        ...
        return (
          <ImpactComp
            type="impact"
            impactSetYear={trainings.impactSetYear}
            impactType={trainings.impactType}
            author="Admin"
            date={date}
  ...
  const updateMultiple = (status) => {
    const options = {
      headers: { "Content-Type": undefined },
      url: `${baseURL}/impact/update-multiple`,
      method: "POST",
      data: {
        ids: isCheck,
        status: status,
  ...
  const deleteAll = () => {
    const options = {
      headers: { "Content-Type": undefined },
      url: `${baseURL}/impact/delete-many`,
      method: "POST",
      data: {
        ids: isCheck,
      },
